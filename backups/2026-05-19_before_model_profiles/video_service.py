...
class VideoGenerator:
    ...
    def generate(
        ...
        *,
        ...
        with self._renderer_session(outputs_link, run_dir / "outputs"):
            rendered_path = self._run_renderer(settings_path, run_dir / "outputs")
    ...
    def _build_runtime_settings(
        ...
        *,
        ...
        with self.settings.wan_template_path.open("r", encoding="utf-8") as handle:
        ...
        runtime = {
            ...
            "base_model_type": self.settings.default_model_type,
            "mode": "t2v",
        ...
        runtime["use_sliding_window"] = runtime["video_length"] > 80
        runtime["sliding_window_size"] = min(runtime["video_length"], 24)
        runtime["sliding_window_stride"] = max(1, runtime["sliding_window_size"] // 2)
        runtime["sliding_window_overlap"] = (
            runtime["sliding_window_size"] - runtime["sliding_window_stride"]
        )
    ...
    def _prepare_outputs_link(self, outputs_link: Path, run_outputs_dir: Path) -> None:
    ...
    @contextmanager
    def _renderer_session(self, outputs_link: Path, run_outputs_dir: Path):
        # Wan2GP uses a shared outputs path, so serialize this section to avoid races
        # between concurrent requests rewriting the same symlink/directory.
        with self._render_lock:
            self._prepare_outputs_link(outputs_link, run_outputs_dir)
            yield

    def _run_renderer(self, settings_path: Path, output_dir: Path) -> Path:
